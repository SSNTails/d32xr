...
static void P_UnlinkBlockmap(mobj_t *thing)
{
	...
	if (thing->bnext)
	{
		mobj_t *bnext = SPTR_TO_LPTR_NN(thing->bnext);
		bnext->bprev = thing->bprev;
	}
	if (thing->bprev)
	{
		mobj_t *bprev = SPTR_TO_LPTR_NN(thing->bprev);
...
static void P_LinkBlockmap(mobj_t *thing, fixed_t x, fixed_t y)
{
	...
	if (blockx>=0 && blocky>=0)
	{
		...
		if (blockx < bmapwidth && blocky <bmapheight)
		{
			...
			thing->bnext = *link;
			if (*link)
				((mobj_t *)(SPTR_TO_LPTR_NN(*link)))->bprev = LPTR_TO_SPTR_NN(thing);
			*link = LPTR_TO_SPTR_NN(thing);
		}
		else
